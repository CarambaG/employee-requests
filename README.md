...
Фильтры можно комбинировать. Список сортируется по номеру заявки в обратном порядке. Изменение исполнителя и статуса будет добавлено отдельным коммитом.
...
`internal/request` содержит создание, получение и фильтрацию заявок. `internal/storage/postgres/request_repository.go` формирует параметризованные SQL-запросы и возвращает заявку вместе с автором, исполнителем и их справочными данными.
