...
## Базовый замер производительности

После генерации миллиона заявок выполните замер запроса до оптимизации:

```bash
make benchmark-baseline
```

Сценарий выбирает конкретного исполнителя с просроченными заявками в статусе `in_progress`, выполняет прогрев и три `EXPLAIN (ANALYZE, BUFFERS)`. План и время сохраняются в `performance-results/baseline.txt`, а все найденные заявки в порядке срока выполнения — в `performance-results/baseline_rows.csv`.

Можно явно задать исполнителя:

```bash
ASSIGNEE_ID=42 make benchmark-baseline
```

Подробная методика и целевой SQL-запрос приведены в [`docs/performance.md`](docs/performance.md). На этом этапе специальный составной индекс не добавлен, чтобы сохранить корректную точку сравнения «до оптимизации».

## Оптимизация целевого запроса

После фиксации базового замера применяется миграция с составным индексом:
...
Порядок полей соответствует условиям запроса: сначала идут два поля с проверкой на равенство, затем поле диапазона и сортировки. Такой индекс позволяет PostgreSQL найти заявки конкретного исполнителя в нужном статусе и прочитать их уже в порядке `due_at`, не выполняя полный просмотр таблицы и отдельную сортировку.

Применить миграцию и повторить замер:

```bash
make benchmark-optimized
```

Если `ASSIGNEE_ID` не указан, сценарий использует идентификатор из `performance-results/baseline.txt`, чтобы сравнение выполнялось на одном исполнителе. Результаты сохраняются в:
...
CSV-файлы остаются локальными и исключены из Git. В репозиторий следует добавить текстовые отчёты `baseline.txt`, `optimized.txt` и `comparison.md`.

`comparison.md` автоматически содержит среднее время трёх измеряемых запусков до и после оптимизации, коэффициент ускорения, снижение времени в процентах и основные операции планов выполнения.
